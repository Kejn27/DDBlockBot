...
void CGameClient::OnConsoleInit()
{
	...
	Console()->Register("menu", "s[cmd] ?s[arg]", CFGFLAG_CLIENT | CFGFLAG_GAME, ConMenu, this, "Menu command");
...
void CGameClient::OnInit()
{
	...
	if(!file)
	{
		BotSettings[SETTING_ADD_IGNORE]   = ".add_ignore";
		BotSettings[SETTING_CLEAR_IGNORE] = ".clear_ignore";
		BotSettings[SETTING_ATTACK]       = ".attack";
		BotSettings[SETTING_IDLE]         = ".idle";
		BotSettings[SETTING_HELPADMIN]    = "1";
		BotSettings[SETTING_DEF]          = ".def";
		BotSettings[SETTING_SIT]          = ".sit";
		BotSettings[SETTING_FOLLOW]       = ".follow";
...
int CGameClient::nearest_character() {
	...
	CClientData OwnClientData = m_aClients[m_LocalIDs[0]];
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(distance(m_LocalCharacterPos, Position) < Distance)
		{
			ClosestID = i;
			Distance = distance(m_LocalCharacterPos, Position);
		}
	}
		...
		if(near != -1)
		{
			...
			if(IntersectCharacter(nearPos, localPos, point, m_LocalIDs[0]) != -1)
			{
				if(!IsFreezeTile(nearPos.x, nearPos.y))
				{
					...
					// Mouse
					if (!AdminInFreeze) {
						m_Controls.m_MousePos[0].x = lerp(m_Controls.m_MousePos[0].x, nearPos.x - localPos.x, 0.5);
						m_Controls.m_MousePos[0].y = lerp(m_Controls.m_MousePos[0].y, nearPos.y - localPos.y, 0.5);
					}
...
void CGameClient::OnMenu(const char* aCmd, const char* aArg) {
	if (str_comp(aCmd, "help") == 0) {
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "=== HELP ===");
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "main - Main menu");
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "setting - Settings menu");
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "info - Info about bot");
	}
	if(str_comp(aCmd, "main") == 0)
	{
		MenuState = MENU_MAIN;

		SaveSettings();

		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "=== MAIN ===");
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "> Settings");
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "> Info");
	}
	if(str_comp(aCmd, "settings") == 0)
	{
		...
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "=== SETTINGS ===");
		if(str_comp(GetSetting(SETTING_HELPADMIN), "1") == 0)
			Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "true | Help admins | help_admin (1 - 0)");
		else Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", "false | Help admins | help_admin (1 - 0)");

		std::string data;
		data = GetSetting(SETTING_ATTACK) + std::string(" | Attack command | attack"); // Attack command
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", data.c_str());
		data = GetSetting(SETTING_IDLE) + std::string(" | Idle command | idle"); // Idle command
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", data.c_str());
		data = GetSetting(SETTING_DEF) + std::string(" | Defend command | def"); // Defend command
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", data.c_str());
		data = GetSetting(SETTING_ADD_IGNORE) + std::string(" | Add to ignore list command | add_ignore"); // Add ignore command
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", data.c_str());
		data = GetSetting(SETTING_CLEAR_IGNORE) + std::string(" | Clear ignore list command | clear_ignore"); // Clear ignore command
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", data.c_str());
		data = GetSetting(SETTING_SIT) + std::string(" | Sit command | sit"); // Sit command
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", data.c_str());
		data = GetSetting(SETTING_FOLLOW) + std::string(" | Follow command | follow"); // Follow command
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-menu", data.c_str());
	}
	if(str_comp(aCmd, "info") == 0)
	{
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-info", "=== INFO ===");
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-info", "Myr - Main code");
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "bot-info", "Rainbow Tee - code");
	}

	if (MenuState == MENU_SETTINGS) {
		if(str_comp(aCmd, "attack") == 0)
			SetSetting(SETTING_ATTACK, aArg);
		if(str_comp(aCmd, "idle") == 0)
			SetSetting(SETTING_IDLE, aArg);
		if(str_comp(aCmd, "def") == 0)
			SetSetting(SETTING_DEF, aArg);
		if(str_comp(aCmd, "add_ignore") == 0)
			SetSetting(SETTING_ADD_IGNORE, aArg);
		if(str_comp(aCmd, "clear_ignore") == 0)
			SetSetting(SETTING_CLEAR_IGNORE, aArg);
		if(str_comp(aCmd, "sit") == 0)
			SetSetting(SETTING_SIT, aArg);
		if(str_comp(aCmd, "follow") == 0)
			SetSetting(SETTING_FOLLOW, aArg);
		if(str_comp(aCmd, "help_admin") == 0)
			SetSetting(SETTING_HELPADMIN, aArg);
	}
}

void CGameClient::SetSetting(int var, const char *val) {
	switch(var)
	{
	case SETTING_ADD_IGNORE:   BotSettings[var].copy((char *)val, 128, 0); break;
	case SETTING_ATTACK:       BotSettings[var].copy((char *)val, 128, 0); break;
	case SETTING_CLEAR_IGNORE: BotSettings[var].copy((char *)val, 128, 0); break;
	case SETTING_DEF:          BotSettings[var].copy((char *)val, 128, 0); break;
	case SETTING_FOLLOW:       BotSettings[var].copy((char *)val, 128, 0); break;
	case SETTING_HELPADMIN:    BotSettings[var].copy((char *)val, 128, 0); break;
	case SETTING_IDLE:         BotSettings[var].copy((char *)val, 128, 0); break;
	case SETTING_SIT:          BotSettings[var].copy((char *)val, 128, 0); break;
	}
}

char* CGameClient::GetSetting(int var) {
	char aBuf[128];

	switch(var)
	{
	case SETTING_ADD_IGNORE:   str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	case SETTING_ATTACK:       str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	case SETTING_CLEAR_IGNORE: str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	case SETTING_DEF:          str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	case SETTING_FOLLOW:       str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	case SETTING_HELPADMIN:    str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	case SETTING_IDLE:         str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	case SETTING_SIT:          str_format(aBuf, 128, "%s", BotSettings[var].c_str()); break;
	}

	return aBuf;
}

void CGameClient::LoadSettings() {
...
void CGameClient::ConMenu(IConsole::IResult *pResult, void *pUserData)
{
	((CGameClient *)pUserData)->OnMenu(pResult->GetString(0), pResult->GetString(1));
}
